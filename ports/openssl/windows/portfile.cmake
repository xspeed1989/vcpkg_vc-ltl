...
if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
    ...
    if(DEFINED VCPKG_IS_LTL AND VCPKG_IS_LTL EQUAL 1)
        set(_BUILD_CMD cmd /c "${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd" && "${JOM}" -k -j "${VCPKG_CONCURRENCY}" -f "${OPENSSL_MAKEFILE}")
        set(_INSTALL_CMD cmd /c "${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd" && "${JOM}" -j 1 -f "${OPENSSL_MAKEFILE}" install_sw install_ssldirs)
        set(CONFIGURE_COMMAND  "${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd" && "${PERL}" Configure ${CONFIGURE_OPTIONS})
    elseif()
        set(_BUILD_CMD "${JOM}" -k -j "${VCPKG_CONCURRENCY}" -f "${OPENSSL_MAKEFILE}")
        set(CONFIGURE_COMMAND  "${PERL}" Configure ${CONFIGURE_OPTIONS})
        set(_INSTALL_CMD "${JOM}" -j 1 -f "${OPENSSL_MAKEFILE}" install_sw install_ssldirs)
    endif()
    ...
    if(DEFINED VCPKG_IS_LTL AND VCPKG_IS_LTL EQUAL 1 AND VCPKG_CRT_LINKAGE STREQUAL "static")
        ...
        message(STATUS ${SOURCE_PATH_RELEASE}\\${OPENSSL_MAKEFILE})
    endif()
