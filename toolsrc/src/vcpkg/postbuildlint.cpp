...
namespace vcpkg::PostBuildLint
{
                                                         ...
                                                         const PreBuildInfo& pre_build_info)
    {
        ...
        for (const fs::path& dll : dlls)
        {
            ...
            std::set<std::string> detectedMsvcrt;
            for (const OutdatedDynamicCrt& outdated_crt : get_outdated_dynamic_crts(pre_build_info.platform_toolset))
            {
                if (std::regex_search(ec_data.output.cbegin(), ec_data.output.cend(), std::regex("msvcrt.dll")))
                {
                    detectedMsvcrt.insert("msvcrt.dll");
                }

                if (std::regex_search(ec_data.output.cbegin(), ec_data.output.cend(), std::regex("msvcp60.dll")))
                {
                    detectedMsvcrt.insert("msvcp60.dll");
				}
            ...
            for (const std::string& crt : detectedMsvcrt)
            {
                System::print2(System::Color::success, "Detected dynamic CRT: ", crt + "\n");
            }
