function(vcpkg_build_nmake)
    ...
    foreach(build_type IN ITEMS debug release)
        if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL build_type)
            ...
            set(PRERUN_SHELL_WRAP ${arg_PRERUN_SHELL})
            set(PRERUN_SHELL_VAR_WRAP ${${prerun_variable_name}})
            set(JOM_WRAP "${JOM}" /K /J ${VCPKG_CONCURRENCY} ${make_opts})
            set(NMAKE_WRAP "${NMAKE}" ${make_opts})
            if(${build_type} STREQUAL "release" AND ${VCPKG_IS_LTL} EQUAL 1)
                if(DEFINED arg_PRERUN_SHELL)
                    set(PRERUN_SHELL_WRAP cmd /c ${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd && ${arg_PRERUN_SHELL})                    
                endif()
                if(DEFINED "${prerun_variable_name}")
                    set(PRERUN_SHELL_VAR_WRAP cmd /c ${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd && ${${prerun_variable_name}})      
                endif()
                set(JOM_WRAP cmd /c ${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd && "${JOM}" /K /J ${VCPKG_CONCURRENCY} ${make_opts})
                set(NMAKE_WRAP cmd /c ${VCPKG_ROOT_DIR}/scripts/VC-LTL_NMake.cmd && "${NMAKE}" ${make_opts})
            endif()

            if(DEFINED arg_PRERUN_SHELL)
                message(STATUS "Prerunning ${triplet_and_build_type}")
                vcpkg_execute_required_process(
                    COMMAND ${PRERUN_SHELL_WRAP}
